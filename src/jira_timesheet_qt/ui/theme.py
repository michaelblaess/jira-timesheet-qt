...
def build_qss(mode: Mode, font_sans: str, font_mono: str) -> str:
    ...
    #PreviewKey {{ font-size: 16px; font-weight: 700; background: transparent; }}
    ...
    #PreviewRule {{ background-color: {p.border}; border: none; }}
    ...
    #PreviewValue[missing="true"] {{ color: {p.text_tertiary}; }}
    #PreviewValue[due="overdue"] {{ color: {p.red}; font-weight: 700; }}
    #PreviewValue[due="soon"] {{ color: {p.orange}; font-weight: 700; }}
    #PreviewTitle {{ color: {p.text_primary}; font-size: 16px; font-weight: 700; background: transparent; }}
    #PreviewBody {{ background-color: {PREVIEW_PAPER}; color: {LIGHT.text_primary}; border: 1px solid {p.border}; border-radius: {RADIUS_SM}px; font-size: 13px; padding: 8px 12px; margin: 12px 14px 6px 14px; selection-background-color: {LIGHT.accent_subtle}; selection-color: {LIGHT.text_primary}; }}
